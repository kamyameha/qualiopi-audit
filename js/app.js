...
function statutProgression(a){const p=progression(a);return p.pct===0?'À faire':p.pct===100?'Terminé':'En cours'}
...
function slugifier(texte='audit'){return texte.normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]+/g,'-').replace(/^-|-$/g,'')||'audit'}
...
function allerAudit(a,indicateur=1,remplacer=false){const nouvelAudit=vue.auditId!==a.id;vue.auditId=a.id;vue.indicateur=indicateur;if(nouvelAudit){vue.filtre='tous';vue.criteresOuverts=[]}const action=remplacer?'replaceState':'pushState';history[action](null,'',location.pathname+location.search+urlAudit(a,indicateur));afficherAudit(false)}
function lireRoute(){const m=location.hash.match(/^#\/audit\/([^/]+)(?:\/[^/]+)?(?:\/indicateur\/(\d+))?/);if(m){const a=donnees.audits.find(x=>x.id===m[1]);if(a){vue.auditId=a.id;vue.indicateur=Math.min(32,Math.max(1,Number(m[2]||1)));afficherAudit(false);return}}afficherAccueil(false)}
...
function configurerEnteteAudit(){actionsEntete.innerHTML='<button class="lien-navigation" id="accueilAudits">Accueil des audits</button><button class="lien-navigation" id="parametresAudit">Paramètres</button><button class="bouton principal" id="nouvelAudit">Créer un audit</button>';document.querySelector('#accueilAudits').onclick=()=>allerAccueil();document.querySelector('#parametresAudit').onclick=ouvrirModification;document.querySelector('#nouvelAudit').onclick=ouvrirCreation}
...
function afficherAccueil(synchroniser=true){
  ...
  const audits=[...donnees.audits].sort((a,b)=>{const ta=progression(a).pct===100||a.statutAudit==='termine',tb=progression(b).pct===100||b.statutAudit==='termine';if(ta!==tb)return ta?1:-1;return Number(b.date?.replaceAll('-','')||b.modifie||0)-Number(a.date?.replaceAll('-','')||a.modifie||0)});
  const lignes=audits.map(a=>`<article class="ligne-audit" data-audit="${a.id}"><h2>${echapper(a.nom)}</h2><p>${formatDate(a.date)}</p><p>${statutProgression(a)}</p><p>${echapper(a.type)}</p></article>`).join('');
  app.innerHTML=`<section class="page"><div class="hero"><div class="hero-gauche"><p class="sur-titre">Suivi de qualité</p></div><div class="hero-droite"><h1>Préparation des audits Qualiopi</h1><p>Centralisez les preuves, suivez l’avancement des 32 indicateurs et gardez<br>une trace claire pour chaque audit de Formation Entre-Deux.</p></div></div><section class="liste-audits"><div class="entete-tableau"><span>Audit</span><span>Date</span><span>Statut</span><span>Type d’audit</span></div>${lignes||`<div class="vide"><h2>Aucun audit pour le moment</h2><p>Créez votre premier audit pour commencer à rassembler les preuves.</p><button class="bouton principal" id="premierAudit">Créer un audit</button></div>`}</section></section>`;
...
function afficherAudit(synchroniser=true){
  const ancienneBarre=document.querySelector('.barre-laterale');const ancienScroll=ancienneBarre?.scrollTop||0;
  ...
  const nav=indicateursFiltres(a).map(c=>{const ouvert=vue.criteresOuverts.includes(String(c.critere));return`<section class="critere-groupe ${ouvert?'ouvert':''}"><button class="critere-nav" data-critere="${c.critere}" aria-expanded="${ouvert}"><span>Critère ${c.critere} — ${c.titre}</span><span class="chevron">⌄</span></button><div class="critere-indicateurs" ${ouvert?'':'hidden'}>${c.indicateurs.map(i=>`<div class="indicateur-nav ${i.n===vue.indicateur?'actif':''}" data-indicateur="${i.n}"><span class="pastille ${a.indicateurs[i.n].etat==='en_cours'?'encours':a.indicateurs[i.n].etat==='termine'?'termine':''}"></span><span><strong>${i.n}.</strong> ${i.texte.slice(0,70)}…</span></div>`).join('')}</div></section>`}).join('')||'<p class="meta">Aucun indicateur ne correspond.</p>';
...
formulaire.addEventListener('submit',e=>{e.preventDefault();if(e.submitter?.value!=='enregistrer')return dialogue.close();const fd=new FormData(formulaire),id=fd.get('auditId');if(id){const a=donnees.audits.find(x=>x.id===id);if(a){a.nom=fd.get('nom');a.date=fd.get('date');a.type=fd.get('type');a.responsable=fd.get('responsable');a.modifie=Date.now();sauver();dialogue.close();afficherAudit();notifier('Paramètres enregistrés')}}else{const a={id:crypto.randomUUID(),nom:fd.get('nom'),date:fd.get('date'),type:fd.get('type'),responsable:fd.get('responsable'),statutAudit:'preparation',reviseLe:null,cree:Date.now(),modifie:Date.now(),indicateurs:creerEtat()};donnees.audits.unshift(a);sauver();dialogue.close();allerAudit(a);notifier('Audit créé')}});
document.querySelector('#retourAccueil').onclick=()=>allerAccueil();
window.addEventListener('popstate',lireRoute);window.addEventListener('hashchange',lireRoute);
async function initialiser(){try{const r=await fetch('./data/qualiopi.json',{cache:'no-store'});if(!r.ok)throw new Error('Référentiel indisponible');REFERENTIEL=await r.json()}catch(e){app.innerHTML='<section class="erreur-chargement"><h1>Impossible de charger le référentiel Qualiopi</h1><p>Rechargez la page ou vérifiez que le dossier <strong>data</strong> a bien été déposé sur GitHub.</p></section>';console.error(e);return}donnees=charger();lireRoute()}
initialiser();